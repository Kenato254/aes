...
class AES:
        ...
        * Function
        ...
        for each in range(4, len(const)+1, 4):
            ...
            for x in range(each-4, each):
                ...
                if x % 4 == 0:
                    c = 0
                mixed.append(self._mixColumnsAdd([self._mixColumnsMult(x, y, 0x80) for x, y in zip(state[start:to], const[c:c+4])]))
                c +=4
    ...
    # Rijndael implementations simply use pre-calculated lookup tables to perform the byte multiplication by 9, 11, 13, and 14.
    #? Multiply by 9 (0x09):
    _mult9 = [
